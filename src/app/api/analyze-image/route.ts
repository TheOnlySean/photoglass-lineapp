...
export async function POST(request: NextRequest) {
  try {
    ...
    // 重新设计的system prompt - 专注于总结和简洁，直接描述内容
...
⭕ 良い例: 「風邪薬の説明書です。1日3回、食後に服用してください。」
...
- 箇条書きではなく自然な文章で`;
    ...
    // 简化的user prompt - 更直接的指示
    const userPrompt = prompt || `内容を直接的に説明してください。前置きは不要です。文字がある場合は要点を要約し、文字がない場合は写っているものを説明してください。`;
